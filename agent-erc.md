...
Furthermore, we also lay out a concrete implementation of an chain agent that runs in a fully on-chain execution environment, and is customizable for any task. We also create a DeFi liquidity pool manager agent for demonstrating how straightforward it is to create your own agent using this framework.
